...
'''Правила просты. Компбюитер загадывает слово, а Вам, мой дорогой друг, нужно его отгадать.
    За каждую неправильную букву Ваш персонаж будет всё ближе к смерти! Так что будь аккуратен!
    
    P.S В строчка №234 добавлена для отладки программы и проверки её работоспособности. Если Вы хотите играть честно, 
    то НЕ не вытаскивайте её из комментария.
    
    Приятной игры!!!'''


def display_hp(tries):
...
def progress(word, word_now, letter, prev_letters, iteration, hp):
    prev_letters.append(letter.upper())
    if letter in word:
        ...
        if iteration == 1:
            print('Первая буква в молоко')
            hp -= 1
        else:
            hp -= 1
    return word_now, prev_letters, hp


def show_progress(word_now, hp, prev_letters, iteration):
    print(f'Картина следующая: \n', ' '.join(word_now))
    if iteration > 1:
        print('Ты уже использовал эти буквы: ', ' '.join(prev_letters))
    print(display_hp(hp))
...
def play():
    ...
    word = choose_word()
    # print(f'Чисто для своих: {word}')  # Для проверки программы эту строку следует раскомментировать
    word_now = ['_' for i in range(len(word))]
    iter += 1
    print(f'Раунд №{iter}')
    letter = input('Какую букву попробуешь первой? \n').upper()
    word_now, letters, hp = progress(word, word_now, letter, letters, iter, hp)
    show_progress(word_now, hp, letters, iter)

    while True:
        iter += 1
        print(f'Раунд №{iter}')
        letter = input('Какую букву попробуешь теперь? \n').upper()
        while not 1040 <= ord(letter) <= 1103:
            letter = input('А введи-ка русскую букву от А до Я: ')
        word_now, letters, hp = progress(word, word_now, letter, letters, iter, hp)
        show_progress(word_now, hp, letters, iter)
        if hp == 0:
            print('Наигрался... GG')
            break
        if ''.join(word_now) == word:
            print('Ого, победа! Поздравляю!')
            break


play()
while True:
    wish = input('Есть желание ещё сыграть? (+ это да, - это нет)')
    if wish == '+':
        play()
    elif wish == '-':
        print('Ну ладно, пока!')
        ...
        print('А может введёшь то, что надо?')
